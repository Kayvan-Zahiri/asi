...
def test_pipeline_horde_ac_forwards_termination_discount() -> None:
    config = AlbertaPipelineConfig(
        features=Step2FeatureConfig.identity(observation_dim=3),
        horde=Step3HordeConfig(
            gammas=(0.9, 0.5),
    ...
    res_nonterminal = pipeline.update(
    ...
    # Terminal value-head TD target must be reward alone (0.5), without bootstrapping
    assert float(res_terminal.horde_td_targets[0]) == 0.5
    # Non-terminal bootstraps with gamma=0.9
    assert float(res_nonterminal.horde_td_targets[0]) != float(res_terminal.horde_td_targets[0])

    # Terminal transition zeroes actor traces across episode boundary
    from alberta_framework.core.horde_actor_critic import HordeActorCriticState

    terminal_ac_state = res_terminal.state.control_state
    assert isinstance(terminal_ac_state, HordeActorCriticState)
    assert bool(jnp.all(terminal_ac_state.actor_trace_weights == 0.0))
