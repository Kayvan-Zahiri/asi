...
def test_observation_scale_normalizes_without_floor() -> None:
    for scale in (1.0, 1e-3, 1e-6, 1e-9, 1e-20):
        ...
        ref_state = ref_model.init(jr.PRNGKey(0))
...
def test_observation_scale_rejects_subnormal_scale() -> None:
    with pytest.raises(ValueError):
        LatentWorldModelConfig(
            observation_dim=1,
            latent_dim=2,
