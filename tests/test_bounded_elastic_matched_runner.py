...
def _run_for_test(data_x: object, data_y: object, *, config: IPMNISTConfig) -> dict[str, object]:
...
def _fake_run(
    ...
    value = (
        0.5
        + float(runner.TEST_ONLY_SEEDS.index(seed) + tuple(runner.ARMS).index(resolved.name))
        / 100.0
    )
...
def test_resource_envelope_uses_task_length_and_rejects_unaccounted_dataset_copy(
    ...
    assert envelope["schedule_bytes"] == SMALL.n_tasks * (SMALL.task_length + SMALL.input_dim) * 4
...
def test_json_preflight_rejects_hostile_scalar_bounds(value: object, message: str) -> None:
...
def test_source_identity_detects_dependency_input_mutation(
    ...
    assert all(
        changed[path] == digest for path, digest in original.items() if path != dependency_input
    )


@pytest.mark.skipif(
    HAS_O_TMPFILE,
    reason="fail-closed descriptor guard is exercised off Linux",
)
def test_bounded_elastic_reserve_output_fails_closed_without_linux_descriptor_support(
    tmp_path: Path,
) -> None:
    with pytest.raises(OSError, match="Linux descriptor support"):
        runner._reserve_output(tmp_path / "out.json", _capability=runner._TEST_EXECUTION_CAPABILITY)
