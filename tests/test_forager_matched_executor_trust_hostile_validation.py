"""Trust-boundary validation for forager_matched_executor sanitized errors."""
...
def test_duplicate_keys_sanitized() -> None:
    pairs = [("a", 1), ("a", 2)]
    with pytest.raises(ForagerMatchedExecutorError, match="duplicate JSON object key") as exc:
...
def test_reject_nonfinite_sanitized() -> None:
    with pytest.raises(ForagerMatchedExecutorError, match="non-finite JSON constant") as exc:
...
def test_parse_float_sanitized() -> None:
    with pytest.raises(ForagerMatchedExecutorError, match="non-finite JSON number") as exc:
        _parse_float("inf")
    msg = str(exc.value)
    assert "!r" not in msg
    assert "'inf'" in msg
...
def test_source_contains_no_repr_leak() -> None:
    p = pathlib.Path("alberta_framework/benchmarks/forager_matched_executor.py")
    text = p.read_text(encoding="utf-8")
    assert "!r" not in text
    assert "duplicate JSON object key '{host_key}'" in text
    assert "non-finite JSON constant '{host_value}' is forbidden" in text
