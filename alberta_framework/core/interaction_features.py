...
class FixedBudgetInteractionLearner:
    ...
    def __init__(
        ...
        task_activity_decay = validated_float32_scalar(
            "task_activity_decay", task_activity_decay, lower=0.0, upper=1.0
        )
        if task_activity_decay >= 1.0:
        ...
        if utility_retention_decay is not None:
            utility_retention_decay = validated_float32_scalar(
                "utility_retention_decay", utility_retention_decay, lower=0.0, upper=1.0
            )
            if not (utility_decay <= utility_retention_decay < 1.0):
        ...
        if candidate_utility_retention_decay is not None:
            ...
            if not (utility_decay <= candidate_utility_retention_decay < 1.0):
                raise ValueError(
        ...
        scale_normalizer_decay = validated_float32_scalar(
            "scale_normalizer_decay", scale_normalizer_decay, lower=0.0, upper=1.0
        )
        if scale_normalizer_decay >= 1.0:
